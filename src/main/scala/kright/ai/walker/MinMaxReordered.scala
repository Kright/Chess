...
/**
	...
	*/
class MinMaxReordered(val maxRecursionDepth: Int) extends Walker {
	...
	override def doTurn(board: Board, metrics: WinFactor): Move = {
		...
		println(s"MINMAX R\tvariantsCount = $variantsCount, probWinWhite = $winChance, time ${System.currentTimeMillis() - t}ms, perf = ${variantsCount * 1000 / dt} pos/sec")
